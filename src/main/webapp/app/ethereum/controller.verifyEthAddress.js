...
// router.js + ( url: '/ethereumVerification/{fingerprint}')
    ...
    var controller = angular.module("cryptonomica.controller.verifyEthAddress", []);
...
// https://docs.angularjs.org/api/ng/provider/$logProvider
    ...
    * */
    ...
    controller.controller("cryptonomica.controller.verifyEthAddress", [
        '$scope',
        '$sce',
        '$rootScope',
        '$http',
        ...
        function verifyEthAddressCtrl($scope,    // <- name for debugging
            ...
            $scope.controllerName = "verifyEthAddressCtrl";
            $log.debug($scope.controllerName, "started"); //
            ...
            $scope.gapiCheck = function () { // implement this pattern in other controllers
                if ($window.gapi && $window.gapi.client) {
                    $log.debug("[" + $scope.controllerName + "] $window.gapi.client:");
                    $log.debug($window.gapi.client);
                    $log.debug("[" + $scope.controllerName + "] starting GAuth.checkAuth()");
                    GAuth.checkAuth().then(function () {
                        ...
                        .then(function (resp) {
                            ...
                            if (!$rootScope.gapi.user) { // user not logged in
                        ...
                        .then(function (resp) {
                            ...
                            if ($scope.userProfileGeneralView.hasOwnProperty("pgpPublicKeyGeneralViews")
                                ...
                                } else if ($scope.userKeys.indexOf($scope.privateKeyInLocalStorageFingerprint) > -1) {
                                    $scope.fingerprint = $scope.privateKeyInLocalStorageFingerprint;
                                } else if ($scope.userKeys.length > 0) {
                                    $scope.fingerprint = $scope.userKeys[0];
                                }
                    ...
                    $log.error("[" + $scope.controllerName + "] $window.gapi.client is not loaded");
                    // setTimeout(gapiCheck, 1000); // check again in a second
                    setTimeout($scope.gapiCheck, 50); //
                }
            };
            $scope.gapiCheck();
